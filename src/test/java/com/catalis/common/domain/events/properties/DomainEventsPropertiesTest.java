...
class DomainEventsPropertiesTest {
    ...
    void kafkaConfigurationShouldHaveCorrectDefaults() {
        ...
        // New Kafka properties defaults
        assertThat(kafka.getBootstrapServers()).isNull();
        assertThat(kafka.getKeySerializer()).isEqualTo("org.apache.kafka.common.serialization.StringSerializer");
        assertThat(kafka.getValueSerializer()).isEqualTo("org.apache.kafka.common.serialization.StringSerializer");
        assertThat(kafka.getRetries()).isNull();
        assertThat(kafka.getBatchSize()).isNull();
        assertThat(kafka.getLingerMs()).isNull();
        assertThat(kafka.getBufferMemory()).isNull();
        assertThat(kafka.getAcks()).isNull();
        assertThat(kafka.getProperties()).isNotNull().isEmpty();
    }
    ...
    void kafkaConfigurationShouldSetAndGetProperties() {
        ...
        additionalProps.put("security.protocol", "SSL");

        // When
        kafka.setTemplateBeanName("customKafkaTemplate");
        kafka.setUseMessagingIfAvailable(false);
        kafka.setBootstrapServers("localhost:9092,localhost:9093");
        kafka.setKeySerializer("org.apache.kafka.common.serialization.ByteArraySerializer");
        kafka.setValueSerializer("org.springframework.kafka.support.serializer.JsonSerializer");
        kafka.setRetries(3);
        kafka.setBatchSize(32768);
        kafka.setLingerMs(100);
        kafka.setBufferMemory(67108864L);
        kafka.setAcks("all");
        kafka.setProperties(additionalProps);

        // Then
        assertThat(kafka.getTemplateBeanName()).isEqualTo("customKafkaTemplate");
        assertThat(kafka.isUseMessagingIfAvailable()).isFalse();
        assertThat(kafka.getBootstrapServers()).isEqualTo("localhost:9092,localhost:9093");
        assertThat(kafka.getKeySerializer()).isEqualTo("org.apache.kafka.common.serialization.ByteArraySerializer");
        assertThat(kafka.getValueSerializer()).isEqualTo("org.springframework.kafka.support.serializer.JsonSerializer");
        assertThat(kafka.getRetries()).isEqualTo(3);
        assertThat(kafka.getBatchSize()).isEqualTo(32768);
        assertThat(kafka.getLingerMs()).isEqualTo(100);
        assertThat(kafka.getBufferMemory()).isEqualTo(67108864L);
        assertThat(kafka.getAcks()).isEqualTo("all");
        assertThat(kafka.getProperties()).containsExactlyInAnyOrderEntriesOf(additionalProps);
    }
