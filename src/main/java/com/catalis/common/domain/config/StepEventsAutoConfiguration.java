...
import com.catalis.common.domain.stepevents.KafkaTemplateStepEventPublisher;
import com.catalis.common.domain.stepevents.NoopStepEventPublisher;
import com.catalis.common.domain.stepevents.RabbitTemplateStepEventPublisher;
...
public class StepEventsAutoConfiguration {
    ...
    public StepEventPublisher stepEventPublisher(ApplicationContext ctx,
        ...
        if (adapter == StepEventsProperties.Adapter.KAFKA) {
            return new KafkaTemplateStepEventPublisher(ctx, props);
        }
        if (adapter == StepEventsProperties.Adapter.RABBIT) {
            return new RabbitTemplateStepEventPublisher(ctx, props);
        }
